  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-24 md:px-8 md:py-32">
      <SectionHeading
        ...
        text="Real plates from real customers. Share yours with #EmberSwat."
        action={
        ...
        {posts.map((n, i) => (
          <Reveal key={n} delay={(i % 6) * 70} direction={i % 2 === 0 ? "scale" : "up"}>
            <a href={brand.socials.instagram} target="_blank" rel="noopener" className="group relative block" aria-label={`Customer post ${n} on Instagram`}>
              <SmartImage src={`/media/social-${n}.jpg`} alt={`Customer food photo ${n}`} label="#EmberSwat" className="aspect-square rounded-2xl" sizes="(max-width:768px) 50vw, 16vw" />
